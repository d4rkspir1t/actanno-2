...
class AAController:
	def __init__(self):
		...
		if cfg.D_PREFIX != "default":
			...
			for id_img_rgb, filename_rgb in enumerate(self.filenames):
				ts_rgb = int(filename_rgb.split("frame")[-1].split(".")[0])
				id_img_depth_closest = 0
				ts_depth_closest = 0
				for id_img_depth, filename_depth in enumerate(self.filenames_depth):
					ts_depth = int(filename_depth.split("frame")[-1].split(".")[0])
	...
	def next_frame_prop_current_rect(self, rect_index):
		...
		if self.cur_frame_nr < len(self.filenames) - 1:
			...
			if trackingLib == None:
				...
				# convert Python types to C types
				c_inrect = to_c_AARect(rect_to_propagate)
				c_outrect = c_AARect()
				...
				# convert C types to Python types
				rect_propagated = to_AARect(c_outrect)
				# self.frames[self.curFrameNr].rects.append(outrect)

			rect_propagated.objectId = propagate_id
	...
	def add_rect(self, x1, y1, x2, y2, objectId, fnr=-1):
		if fnr == -1:
			fnr = self.cur_frame_nr
		if fnr >= len(self.frames):
			raise Exception()
		self.frames[fnr].get_rects().append(AARect(x1, y1, x2, y2, objectId))
	...
	def update_object_id(self, index_rect, new_id):
		self.frames[self.cur_frame_nr].rects[index_rect].objectId = new_id
			...
			for (i, f) in enumerate(self.frames):
				for (j, r) in enumerate(f.getRects()):
					if r.objectId == cur_object_id + 1:
						if not found_rects:
							found_rects = True
							fd.write("	<object nr=\"" + str(cur_object_id + 1) + "\" class=\"" + str(self.class_assignations[cur_object_id]) + "\">\n")
...
class Example(Frame):
	...
	def update_after_jump(self):
		self.cur_frame = ImageTk.PhotoImage(self.img)
		self.display_anno()
		self.parent.title(TITLE +" (frame nr." + str(self.ct.cur_frame_nr + 1) + " of " + str(len(self.ct.filenames)) + ")")
		self.canvas.update()
	...
	def mouse_move(self, event):
		...
		if self.state == "d":
			...
			self.canvas.create_rectangle(self.curx1, self.cury1, self.curx1 + 30, self.cury1 + 30, outline="white", fill="white")
	...
	def chose_object_id(self, event, id):
		sempos = self.ct.get_sem_mouse_pos(self.mousex, self.mousey)
		print "choseobjectId(self,event,id):", sempos.index, "pos: ", self.mousex, ",", self.mousey
		if sempos.index >- 1:
			self.ct.update_object_id(sempos.index, id)
			self.display_anno()
			self.display_class_assignations()
			self.is_modified = True

	# draw an anchor point at (x, y) coordinates
	def draw_anchor_point(self, draw, x, y, size=5, color="cyan"):
		x1 = x-size
		y1 = y-size
		x2 = x+size
		y2 = y+size
